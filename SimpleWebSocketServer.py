...
class WebSocket(object):
   def __init__(self, server, sock, address):
      ...
      self.closed = False
      self.sendq = deque()
      self.messagein = queue.Queue()
      self.messageout = queue.Queue()
   ...
   def handleMessage(self):
      ...
      self.messageout.put(self.data, False)
      ...
      while True:
         ...
         for fileno in self.listeners:
            try:
               ...
               try:
                   msgtoprocess = self.inq.get(True, 0.1)
                   client.sendMessage(msgtoprocess)
         ...
         for ready in rList:
            if ready == self.serversocket:
               try:
                  ...
                  self.connections[fileno].messagein = self.inq
                  self.connections[fileno].messageout = self.outq
               except Exception as n:
                  if sock is not None:
                     sock.close()
            else:
